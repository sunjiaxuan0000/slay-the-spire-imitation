 ...
 *   2) 稀有度飘带：normal.png(weight=4 白卡) / rare.png(3 蓝卡) / gold.png(2 金卡)
 *      —— 卡的类型文字（攻击/技能/能力）写在飘带中间
 *   3) 卡名：卡上方
 *   4) 描述：卡下方
 *   5) 能量徽章 power.png：左上角，能量数字写在徽章上
 *
 * ★ 想调某层贴图位置：改下方 “布局参数” 区对应常量即可。
 */
public class CardFaceView {
    ...
    private static final double BADGE_Y = 6;
    private static final double BADGE_SIZE = 42;   // 徽章边长
    // ======================================================

    private static Image img(String name) {
        var in = CardFaceView.class.getResourceAsStream("icons/" + name);
        return in == null ? null : new Image(in);
    }

    private static Image FACE_ATTACK = img("attack.png");
    private static Image FACE_SKILL = img("skill.png");
    private static Image FACE_ABILITY = img("ability.png");
    private static Image RIBBON_COMMON = img("normal.png"); // 4 白
    private static Image RIBBON_RARE = img("rare.png");     // 3 蓝
    private static Image RIBBON_GOLD = img("gold.png");     // 2 金
    private static Image POWER = img("power.png");
    ...
    public static Pane build(Card c) {
        ...
        typeLabel.setLayoutY(EMBLEM_CY - 22);
